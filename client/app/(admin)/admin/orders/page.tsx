  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }} className="text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white">
          <option value="">All Statuses</option>
          {statuses.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
        </select>
        <Button variant="outline" size="sm"><Filter className="w-3.5 h-3.5" /> More Filters</Button>
        <div className="ml-auto text-xs text-gray-500">{meta.total || 0} total orders</div>
      </div>
